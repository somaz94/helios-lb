...
Guide for building, testing, and contributing to helios-lb.
...
│   └── main.go                       # Entry point
├── api/
│   └── v1/                           # HeliosConfig CRD types
├── internal/
│   ├── controller/                   # Reconciliation logic
│   ├── loadbalancer/                 # LB algorithms & IP allocation
...
│   └── samples/                      # Sample HeliosConfig YAMLs
├── helm/
│   └── helios-lb/                    # Helm chart
├── hack/                             # Helper scripts
│   ├── bump-version.sh
│   ├── find-free-ip.sh
│   └── test-integration.sh
...
| `api/v1/` | HeliosConfig CRD type definitions |
| `internal/controller/` | Kubernetes reconciliation logic |
| `internal/loadbalancer/` | Load balancing algorithms and IP allocation |
| `internal/network/` | Network interface and ARP management |
...
make find-free-ip         # Scan for available IPs before testing
```
...
```bash
make docker-build IMG=somaz940/helios-lb:v0.2.6
make docker-push
...
make branch name=ip-allocation             # Create feature branch from main
make pr title="feat: add IP allocation"    # Test → push → create PR
```
