...
.PHONY: find-free-ip
find-free-ip: ## Scan for available IPs before running integration tests. Use HELP=1 for usage.
	@if [ "$(HELP)" = "1" ]; then bash hack/find-free-ip.sh --help; else bash hack/find-free-ip.sh $(START_IP) $(END_IP) $(COUNT); fi
...
docker-buildx-tag: ## Build and push docker image for the manager for cross-platform support with specific version
	...
	- $(CONTAINER_TOOL) buildx create --name helios-lb-builder
	$(CONTAINER_TOOL) buildx use helios-lb-builder
	# Build and push version-specific tag
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) \
		$(DOCKER_BUILD_ARGS) \
		--tag ${IMG} \
		-f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm helios-lb-builder
	rm Dockerfile.cross
